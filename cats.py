...
def about(topic):
    ...
    def checker(paragraph):
        ...
        while i < len(para):
            ...
            i += 1
        return False
        # see if paragraph contains words from topic
...
def accuracy(typed, reference):
    ...
    while i < min(len(reference_words), len(typed_words)):
        if typed_words[i] == reference_words[i]:
            correct += 1
        i += 1
    return correct*100 / (len(reference_words) + (len(typed_words)-len(reference_words)))
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for f in valid_words:
        if typed_word == f:
            return typed_word
    dic = {word : diff_function(typed_word, word, limit) for word in valid_words}
    ans = min(dic.values()) 
    res = [key for key in dic if dic[key] == ans]
    return res[0] if diff_function(typed_word, res[0], limit) <= limit else typed_word
    # END PROBLEM 5
...
def sphinx_switches(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(startingPoint, remainder):
        if startingPoint < len(start) and remainder >= 0 and startingPoint < len(goal):
            if start[startingPoint] == goal[startingPoint]:
                return helper(startingPoint+1, remainder)
            else:
                return 1 + helper(startingPoint+1, remainder-1)
...
def pawssible_patches(start, goal, limit):
    ...
    if start == goal:  # Fill in the condition
        ...
        add = 1 + pawssible_patches(goal[0] + start, goal, limit-1)
        remove = 1 + pawssible_patches(start[1:], goal, limit-1)
        substitute = 1 + pawssible_patches(goal[0] + start[1:], goal, limit-1)
        # BEGIN
        "* YOUR CODE HERE *"
